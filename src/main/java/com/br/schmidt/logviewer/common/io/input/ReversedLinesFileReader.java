package com.br.schmidt.logviewer.common.io.input;
...
public class ReversedLinesFileReader implements Closeable {
			...
			if (no > 1) {
				...
				if (leftOver != null) {
					throw new IllegalStateException(
							"Unexpected leftover of the last block: leftOverOfThisFilePart=" + new String(leftOver,
									encoding));
				}
				return null;
			}
	...
	public ReversedLinesFileReader(final File file) throws IOException {
		this(file, 4096, Charset.defaultCharset().toString(), null);
	}

	public ReversedLinesFileReader(final RandomAccessFile reader) throws IOException {
		this(null, 4096, Charset.defaultCharset().toString(), reader);
	}
	 ...
	 * @param encoding  the encoding of the file
	 * @throws IOException                 if an I/O error occurs
	 * @throws UnsupportedCharsetException thrown instead of {@link UnsupportedEncodingException} in version 2.2 if
	 *                                     the encoding is not
	 *                                     supported.
	 */
	public ReversedLinesFileReader(final File file, final int blockSize, final String encoding,
			...
			throws IOException {
		this(file, blockSize, Charsets.toCharset(encoding), reader);
	}
	...
	public int getPosition() {
		if (currentFilePart != null) {
			return currentFilePart.currentLastBytePos;
		}
		return 0;
	}
